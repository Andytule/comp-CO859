...
AS
	...
	SET sales_ytd = (SELECT SUM(amount) 
					FROM sales 
					WHERE service_id = (SELECT service_id FROM inserted))
	WHERE service_id = (SELECT service_id FROM inserted);
GO
...
AS
	...
	SET sales_ytd = (SELECT SUM(amount) 
					FROM sales 
					WHERE service_id = (SELECT service_id FROM deleted))
	WHERE service_id = (SELECT service_id FROM deleted);
GO
...
INSERT INTO sales VALUES(31, '2020-11-27', 1500.00, 200);
PRINT 'After INSERT'
SELECT * FROM shiki_shoe_services
DELETE FROM sales WHERE sales_id = 31;
PRINT 'After DELETE'
SELECT * FROM shiki_shoe_services
UPDATE sales SET amount = 1 WHERE sales_id = 30;
PRINT 'After UPDATE'
SELECT * FROM shiki_shoe_services